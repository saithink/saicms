<template>
  <component
    is="a-modal"
    :width="tool.getDevice() === 'mobile' ? '100%' : '1024px'"
    ...
    <a-form ref="formRef" :model="formData" :rules="rules" :auto-label-width="true">
      ...
      <a-form-item label="文章简介" field="describe">
        <a-input v-model="formData.describe" placeholder="请输入文章简介" />
      </a-form-item>
...
let title = computed(() => {
  return '文章管理' + (mode.value == 'add' ? '-新增' : '-编辑')
})
...
// 验证规则
const rules = {
  category_id: [{ required: true, message: '文章分类必需填写' }],
